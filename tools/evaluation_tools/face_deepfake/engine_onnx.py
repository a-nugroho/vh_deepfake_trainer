...
class DeepfakeEngine:
    ...
    def __init__(self, device_name: str = "cpu", use_triton=False):
        ...
        model_path = (
            Path(__file__).parent / "weights/convnext_xlarge_384_in22ft1k_30.pth.onnx"
        )
    ...
    def _run_inference(self, inp_tensor: np.ndarray) -> float:
        ...
        if self.use_triton:
            inputs = []
            outputs = []
            inputs.append(grpcclient.InferInput("input", [1, 3, 384, 384], "FP32"))
...
class DeepfakeEngine_Generic:
    ...
    def __init__(self, device_name: str = "cpu", use_triton=False,
        ...
        model_path = (
            Path(__file__).parent / f"weights/{model_path}"
        )
