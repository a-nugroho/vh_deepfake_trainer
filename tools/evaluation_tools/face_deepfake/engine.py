...
from face_deepfake.network.models import get_convnext
...
def load_network(network, save_filename):
    stdict = torch.load(save_filename, map_location="cpu")
    network.load_state_dict(stdict)
...
class DeepfakeEngine:
    ...
    INPUT_SIZE = (384, 384)
    _INPUT_MEAN = [0.485, 0.456, 0.406]
    _INPUT_STD = [0.229, 0.224, 0.225]
    ...
    def __init__(self, device_name: str = "cpu"):
        ...
        Args:
            device_name (str, optional): The device to use for inference. Defaults to "cpu".
                Options are "cpu" for CPU and "cuda" for CUDA.

        This initializes a PyTorch model instance with the model specified by the model path.
        If CUDA is not available or fails, it defaults to using the CPU.
        """
        model_path = (
            Path(__file__).parent / "weights/convnext_xlarge_384_in22ft1k_30.pth"
        )

        self._model_name = "convnext_xlarge_384_in22ft1k"
        self._model_path = model_path
        self.device_name = device_name

        self.net = get_convnext(
            model_name=self._model_name, num_classes=2, pretrained=False
        )

        self.net = ModuleWrapper(self.net)
        self.net = load_network(self.net, self._model_path).to(device_name)
    ...
    def _run_inference(self, inp_tensor: np.ndarray) -> float:
        ...
        pred: torch.Tensor = self.net(inp_tensor)
    ...
    def predict(self, img: np.ndarray) -> float:
        ...
        pred = self._run_inference(inp_tensor)
