...
def read_data(root_json, name_dataset):
    path_json = os.path.join(root_json,name_dataset+".json")
...
def read_args():
    ...
    # Example arguments
    parser.add_argument("--name_dataset", "-d", required=True, help="Dataset name")
...
def main():
    ...
    args = read_args()
    # Access them
    print("Dataset name:", args.name_dataset)
    data_json = read_data(root_json, args.name_dataset)
    
    list_keys = list(data_json.keys())
    path_result = f"result_{args.name_dataset}.json"
    ...
    for id_now,key_now in enumerate(tqdm(list_keys)):
        if key_now in data_result:
            continue
        else:
            dict_source = data_json[key_now]
            path_img = data_json[key_now]["processed_path"]
            dict_result = dict_source
