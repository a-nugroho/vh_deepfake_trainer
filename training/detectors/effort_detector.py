...
class EffortDetector(nn.Module):
    def __init__(self, config=None):
        ...
        self.backbone = apply_svd_residual_to_self_attn(self.backbone, r=1024-1)
        self.num_classes = config["num_classes"]
        self.loss_func = self.build_loss(config)
        n_features = self.backbone.n_features
        self.head = nn.Linear(n_features, self.num_classes)
...
class EffortDetector(nn.Module):
    ...
    def build_backbone(self, config):
        ...
        clip_model = CLIPModel.from_pretrained("FLIP-dataset/FLIP-large-14", use_safetensors=True)
