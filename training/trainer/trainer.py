...
def universal_round(x, decimals=0):
    if isinstance(x, float):
        return round(x, decimals)
    elif isinstance(x, np.ndarray) or isinstance(x, np.floating):
...
class Trainer(object):
    ...
    def save_ckpt(self, phase, dataset_key,ckpt_info=None,save_latest=False):
            ...
            if iteration % 300 == 0 and self.config['local_rank']==0:
                ...
                for k, v in train_recorder_loss.items():
                    ...
                    if v_avg == None:
                        ...
                        continue
                    loss_str += f"training-loss, {k}: {v_avg:.3f}    "
                    # tensorboard-1. loss
        ...
        for batch_idx, (input_live, input_fake, target_live, target_fake, *additional_input) in enumerate(tqdm(train_data_loader)):
            ...
            if iteration % 300 == 0 and self.config['local_rank']==0:
                if self.config['SWA'] and (epoch>self.config['swa_start'] or self.config['dry_run']):
                    self.scheduler.step()
                # info for loss
                loss_str = f"Iter: {step_cnt} out of {last_idx+1}    "
                for k, v in train_recorder_loss.items():
                    v_avg = v.average()
                    if v_avg == None:
                        loss_str += f"training-loss, {k}: not calculated"
                        continue
                    loss_str += f"training-loss, {k}: {v_avg:.5f}    "
                    # tensorboard-1. loss
                    writer = self.get_writer('train', ','.join(self.config['train_dataset']), k)
                    writer.add_scalar(f'train_loss/{k}', v_avg, global_step=step_cnt)
                self.logger.info(loss_str)
                ...
                for k, v in train_recorder_metric.items():
                    ...
                    if v_avg == None:
                        metric_str += f"training-metric, {k}: not calculated    "
                        continue
                    metric_str += f"training-metric, {k}: {v_avg:.5f}    "
                    # tensorboard-2. metric
                    writer = self.get_writer('train', ','.join(self.config['train_dataset']), k)
                    writer.add_scalar(f'train_metric/{k}', v_avg, global_step=step_cnt)
                self.logger.info(metric_str)
                ...
                self.save_ckpt('train', ','.join(self.config['train_dataset']), f"{epoch}-{iteration}",save_latest=True)
                """
                save_dir = os.path.join(self.log_dir, phase, dataset_key)
                os.makedirs(save_dir, exist_ok=True)
                ckpt_name = f"ckpt_best.pth"
    ...
    def test_one_dataset_verihubs(self, data_loader):
        # define test recorder
        test_recorder_loss = defaultdict(Recorder)
        prediction_lists = []
        feature_lists=[]
        label_lists = []
        for i, (input, target, *additional_input) in tqdm(enumerate(data_loader),total=len(data_loader)):
            data_dict = {}
            data_dict['image'] = input
            data_dict['label'] = target
            data_dict['label'] = torch.where(data_dict['label']!=0, 1, 0)  # fix the label to 0 and 1 only
            ...
            if type(self.model) is not AveragedModel:
                ...
                # store data by recorder
                for name, value in losses.items():
                    test_recorder_loss[name].update(universal_round(value,5))
    ...
    def test_epoch_verihubs(self, epoch, iteration, test_data_loaders, step):
        # set model to eval mode
        self.setEval()
        ...
        for key in keys:
            ...
            # compute loss for each dataset
            losses_one_dataset_recorder, predictions_nps, label_nps, feature_nps = self.test_one_dataset_verihubs(test_data_loaders[key])
            # print(f'stack len:{predictions_nps.shape};{label_nps.shape};{len(data_dict["image"])}')
            losses_all_datasets[key] = losses_one_dataset_recorder
            
            #DEBUG
            img_names = data_dict['image']
            img_names = data_dict['image'][:len(predictions_nps)]

            #print(f"{len(label_nps)} {label_nps.sum()}")
            metric_one_dataset=get_test_metrics(y_pred=predictions_nps,y_true=label_nps,
            img_names=img_names,direct_auc=True)
            for metric_name, value in metric_one_dataset.items():
                if metric_name in avg_metric:
                    avg_metric[metric_name]+=value
            avg_metric['dataset_dict'][key] = metric_one_dataset[self.metric_scoring]
